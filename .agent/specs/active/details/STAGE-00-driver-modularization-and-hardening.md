...
The forensic codebase audit (including [AUDIT-optimizer-pipeline-honesty-2026-09-05.md](file:///c:/Users/ksvik/Projects/Agam-Lang/docs/AUDIT-optimizer-pipeline-honesty-2026-09-05.md)) surfaced critical architectural technical debt:
...
### 2.7 Differential Testing Pipeline (JIT vs. AOT) & Core Backend Parity
...
- Remediate the 5 core lowering gaps catalogued in `.agent/notes/compiler_backend_parity_gaps.md`:
  1. Range iterator desugaring in HIR (`(start..end).reduce(...)` desugars to canonical loop).
  2. Cranelift JIT string concatenation runtime invocation (`agam_str_concat` instead of pointer `iadd`).
  3. Cranelift JIT array aggregate store support (`Op::StoreIndex`).
