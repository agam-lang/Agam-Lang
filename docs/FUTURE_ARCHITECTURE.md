...
| **Layer 4: Mid-Level SSA (MIR)** | Target-independent SSA CFG, GVN, SCCP, escape analysis | 🟢 `[Implemented]` | Full SSA CFG lowering in `crates/middle/agam_mir/` with GVN, constant fold, inliner, and escape promotion. Escape analysis and stack promotion are actively wired into the fixed-point optimization loop (`optimize_module`) with verified conservative scalar promotion and post-dominator drop glue. |
