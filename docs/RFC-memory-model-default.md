...
|---|---:|---|---|---|
| Public memory-model contract | 🟢 | `docs/MEMORY_MODEL.md:9-14,25-57` defines ARC/CoW as default and lexical affine `strict` as Tier 2; version is `0.1.0-alpha.1` at `:3-4`. | The intended two-tier contract is unambiguous. | Documentation is not evidence that the parser, SEMA, MIR, and backends honor the contract. |
...
| Strict/ARC boundary crossing and CoW semantics | ⚪ | The specification promises ARC bypass in strict (`docs/MEMORY_MODEL.md:51-57`), but no parsed boundary or MIR ownership operation represents the transition. |
...
7. What allocator and destructor model makes scoped arenas from `docs/MEMORY_MODEL.md:60-66` compatible with strict drops, FFI, and escape promotion?
